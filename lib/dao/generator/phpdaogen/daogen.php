...
if (!class_exists('Connection', true)) include(dirname(__FILE__).'/Connection.php');
if (!class_exists('MySQLConnection', true)) include(dirname(__FILE__).'/MySQLConnection.php');
if (!class_exists('PreparedStatement', true)) include(dirname(__FILE__).'/PreparedStatement.php');
...
function outputValueObject($tableName, $fields, $primaryKey) {
	...
	// Open the class.
	$code = "<?php\n";
	$code .= "namespace lib\\datamodel\\sql;\n";
	...
	// Getters
	foreach ($fields as $field) {
		$code .= "\tpublic function get".ucwords($field->name)."() {\n";
	...
	// Setters
	foreach ($fields as $field) {
		$code .= "\tpublic function set".ucwords($field->name)."(\$$field->name) {\n";
	...
	$code .= "}\n";
	file_put_contents("$outputDir/$uTableName.php", $code);
}
...
function outputDataAccessObject($tableName, $fields, $primaryKey) {
	...
	// Open the class.
	$code = "<?php\n";
	$code .= "namespace lib\\dao\\sql;\n";
	$code .= "use lib\\datamodel\\sql\\$uTableName;";
	$code .= $GENERATED_HEADER;
	...
	$code .= "\tpublic function __construct(".'$connection'.") {\n";
	...
	// Close the class and write out the php file.
	$code .= "}\n";
	file_put_contents("$outputDir/$uTableName"."Dao.php", $code);
}
...
foreach (glob($scriptDir.'/*.php') as $srcFile) {
